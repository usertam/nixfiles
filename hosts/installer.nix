...

{
  ...
  services.getty.helpLine = lib.mkForce (lib.trimWith { end = true; } ''
    ...
    Assume you want to format ZFS, do        To create the ZFS partitions, do
        ...
        sgdisk -n 1:0:-2G -t 1:BF00 \            zfs create -o mountpoint=legacy zroot/home 
               -n 2:0:0   -t 2:EF00 \            zfs create -o mountpoint=legacy zroot/nix
               /dev/nvme0n1
        ...
        zpool create \                           mount -t zfs zroot/root /mnt
          ...
          zroot /dev/nvme0n1p1                   mount /dev/disk/by-label/ESP /mnt/boot

    To auto-generate hardware configs in /mnt, do
        nixos-generate-config

    To perform a install from nixfiles, do
